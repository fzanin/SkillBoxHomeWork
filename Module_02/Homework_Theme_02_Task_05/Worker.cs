...
namespace Homework_Theme_02_Task_05
{
    ...
    class Worker
    {
        ...
        {
            string outputText = "Name: ";                           // prepare output text
            SetCursorPosition(outputText);                          // set output position
            Console.Write(outputText);                              // Request to input Name
        ...
        public void InputWorkerAge()
        {
            ...
            {
                string outputText = "Age: ";                                            // prepare outbut text
                SetCursorPosition(outputText);                                          // set output position
                Console.Write(outputText);                                              // Request to input Age
                ...
                if (this.Age < this.MinAgeLimit || this.Age > this.MaxAgeLimit)         // Check if age out of range Min - Max
                {
                    ...
                    outputText = $"Worker age must be in the ranage {this.MinAgeLimit} - {this.MaxAgeLimit}. Please repeat input. ";    // prepare output text
                    SetCursorPosition(outputText);                                      // set output position
                    Console.WriteLine(outputText);                                      // show warning message
                }
            }
        }
        ...
        public void InputWorkerHeight()
        {
            ...
            {
                string outputText = "Height: ";                                             // prepare outbut text
                SetCursorPosition(outputText);                                              // set output position
                Console.Write(outputText);                                                  // Request to input Height
                ...
                if (this.Height < this.MinHeightLimit || this.Height > this.MaxHeightLimit) // Check if Height out of range Min - Max
                {
                    ...
                    outputText = $"Worker Height must be in the ranage {this.MinHeightLimit} - {this.MaxHeightLimit}. Please repeat input. ";    // prepare output text
                    SetCursorPosition(outputText);                                          // set output position
                    Console.WriteLine(outputText);                                          // show warning message
                }
            }
        ...
        public void InputWorkerScoreRussian()
        {
            ...
            {
                string outputText = "Score Russian: ";                                              // prepare outbut text
                SetCursorPosition(outputText);                                                      // set output position

                Console.Write(outputText);                                                          // Request to input Score Russian
                ...
                if (this.ScoreRussian < this.MinScoreLimit || this.ScoreRussian > this.MaxScoreLimit)   // Check if Score Russian out of range Min - Max
                {
                    ...
                    outputText = $"Worker Score Russian must be in the ranage {this.MinScoreLimit} - {this.MaxScoreLimit}. Please repeat input. ";      // prepare output text
                    SetCursorPosition(outputText);                                                  // set output position
                    Console.WriteLine(outputText);                                                    // show warning message
                }
            }
        ...
        public void InputWorkerScoreHistory()
        {
            ...
            {
                string outputText = "Score History: ";                                              // prepare outbut text
                SetCursorPosition(outputText);                                                      // set output position

                Console.Write(outputText);                                                          // Request to input ScoreHistory
                ...
                if (this.ScoreHistory < this.MinScoreLimit || this.ScoreHistory > this.MaxScoreLimit)   // Check if ScoreHistory out of range Min - Max
                {
                    ...
                    outputText = $"Worker Score History must be in the ranage {this.MinScoreLimit} - {this.MaxScoreLimit}. Please repeat input. "; // prepare output text
                    SetCursorPosition(outputText);                                                  // set output position
                    Console.WriteLine(outputText); // show warning message
                }
            }
        ...
        public void InputWorkerScoreMathematics()
        {
            ...
            {
                string outputText = "Score Mathematics: ";                                              // prepare outbut text
                SetCursorPosition(outputText);                                                          // set output position
                Console.Write(outputText);                                                              // Request to input ScoreMathematics
                ...
                if (this.ScoreMathematics < this.MinScoreLimit || this.ScoreMathematics > this.MaxScoreLimit)   // Check if ScoreMathematics out of range Min - Max
                {
                    ...
                    outputText = $"Worker Score Mathematics must be in the ranage {this.MinScoreLimit} - {this.MaxScoreLimit}. Please repeat input. ";  // prepare output text
                    Console.WriteLine(outputText); // show warning message
                }
            }
        }
        ...
        /// <returns></returns>
        private void SetCursorPosition(string outputString)
        {
            Console.CursorLeft = Console.WindowWidth / 2 - outputString.Length / 2;       // calculate and set position of cursor
        }
